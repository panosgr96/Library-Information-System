...
public class BrandsUpd extends PopUpJpanel {
    public BrandsUpd() {
        initComponents();
    ...
    private void SaveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SaveButtonActionPerformed
        if (this.getAction() == PopUpJpanel.FormActionType.Insert)
        {
            ...
            String sql ="insert into BRANDS ( NAME ) VALUES ('"+this.TXTNAME.getText()+"')";
            try{
                con.UpdateQuery(sql);
                con.Close();
            }
        ...
        {
            ConnectDb con = new ConnectDb();
            String sql ="UPDATE BRANDS SET NAME ='"+this.TXTNAME.getText()+"' WHERE BRANDSID = "+this.TXTBRANDSID.getText();
            try{
                con.UpdateQuery(sql);
                con.Close();
            }
        ...
        {
            ConnectDb con = new ConnectDb();
            String sql ="DELETE FROM BRANDS WHERE BRANDSID = "+this.TXTBRANDSID.getText();
            try{
                con.UpdateQuery(sql);
                con.Close();
            }
    ...
    private void formAncestorAdded(javax.swing.event.AncestorEvent evt) {//GEN-FIRST:event_formAncestorAdded
        ConnectDb con = new ConnectDb();
        String sql = "SELECT BRANDSID , NAME FROM BRANDS WHERE BRANDSID = ?";
        String[] parameter = new String[1];
        parameter[0] = this.getPKValue();
        try{
            ResultSet rs =  con.ExecuteQuery(sql,parameter);
            while(rs.next()){
              this.TXTBRANDSID.setText(rs.getString("BRANDSID"));
